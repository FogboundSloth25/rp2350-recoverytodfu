...
bool bus_open_ep0(bus_t *b, uint8_t maxpacket) {
    ...
    if (!pio_usb_host_endpoint_open(
            0, b->dev->address, (const uint8_t *)&ep0, false)) {
        return false;
    }
...
int bus_control_xfer(bus_t *b, const uint8_t setup[8],
                     ...
                     bool data_in, uint32_t timeout_ms) {
    ...
    b->dev->control_pipe.out_data_packet.tx_address =
        (!data_in && has_data) ? data : NULL;
    b->dev->control_pipe.out_data_packet.tx_length =
        (!data_in && has_data) ? data_len : 0;
    ...
    while (b->dev->control_pipe.operation != CONTROL_COMPLETE &&
           b->dev->control_pipe.operation != CONTROL_ERROR) {
